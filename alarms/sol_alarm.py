...
from keyboards.alarm_buttons import solscan_button
...
async def sol_data_alarm(data_for_alarm: dict) -> None:
    sol_row = data_for_alarm['row']
    hash_tx = data_for_alarm['hash']  
    
    url = f'https://solscan.io/tx/{hash_tx}'
    url_acc = f'https://solscan.io/account/{sol_row}'
    
    data_for_alarm['url'] = url
    data_for_alarm['url_acc'] = url_acc
    
    short_url = f"{sol_row[:6]}...{sol_row[-4:]}"
    data_for_alarm['short_url'] = short_url
    
    await alarm_to_chat(data_for_alarm)

async def alarm_to_chat(data_for_alarm: dict) -> None:
    button = solscan_button(data_for_alarm['url'])
    ...
    try:
        await bot.send_message(chat_id=data_for_alarm['chat_id'],
                            text=f'🔔 <b>Transaction is found!</b>\n\n'
                                    ...
                                    f"💎 Value: <code>{data_for_alarm['amount_tx']}</code> SOL",
                            reply_markup=button,
