...
from keyboards.alarm_buttons import etherscan_button
...
async def eth_data_alarm(data_for_alarm: dict) -> None:
    eth_row = data_for_alarm['row']
    hash_tx = data_for_alarm['hash'] 
    
    url = f'https://etherscan.io/tx/{hash_tx}'
    url_acc = f'https://etherscan.io/address/{eth_row}'
    
    data_for_alarm['url'] = url
    data_for_alarm['url_acc'] = url_acc
    
    short_url = f"{eth_row[:6]}...{eth_row[-4:]}"
    data_for_alarm['short_url'] = short_url
    
    await alarm_to_chat(data_for_alarm)

async def alarm_to_chat(data_for_alarm: dict) -> None:
    button = etherscan_button(data_for_alarm['url'])
    ...
    try:
        await bot.send_message(chat_id=data_for_alarm['chat_id'],
                            text=f'🔔 <b>Transaction is found!</b>\n\n'
                                    ...
                                    f"💎 Value: <code>{data_for_alarm['amount_tx']}</code> ETH",
                            reply_markup=button,
